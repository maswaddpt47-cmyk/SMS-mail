...
Constat vérifié le 20/08/2026 : le repo `maswaddpt47-cmyk/sms-mail` était **public**, et le
dossier `backups/` — contenant les exports JSON complets de l'historique (nom, prénom,
téléphone, démarche, date/heure de RDV, statut de vrais usagers) — était commité en clair
...
1. ✅ **Fait** — Repo **privé** dédié créé (`sms-mail-backups`), `githubPush()` / `GH_REPO`
   ...
   GitHub". Le repo public actuel garde uniquement le code (nécessaire pour GitHub Pages
   gratuit), plus aucune donnée personnelle n'y est poussée.
2. ✅ **Fait** — Historique git purgé (`git filter-repo --path backups --invert-paths
   --force`) sur les branches `main` et `backups`, puis `push --force` sur les deux.
